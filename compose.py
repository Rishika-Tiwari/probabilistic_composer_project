...
def get_words_from_text(text_path):
    with open(text_path, 'rb') as file:
        ...
        text = text.lower()                                                 # turn everything lowercase
...
def make_graph(words):
    ...
    # for each word
    for word in words:
        # check that word is in graph,if not then add it
        word_vertex = g.get_vertex(word)

        # if there was a previous word, add an edge if does not exist
...
def main():
    words = get_words_from_text('texts/atomic_habits.txt')
